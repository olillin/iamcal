 ...
 * @param {string} [name] An optional filename
 * @returns {Promise<string>} The filename of the created calendar in the form "<eventCount>-events.ics".
 */
async function generateEvents(eventCount, name) {
    ...
    const filename = name ?? `${eventCount}-events.ics`
    await dump(calendar, filename)
    return filename
}

generateEvents(1_000)
